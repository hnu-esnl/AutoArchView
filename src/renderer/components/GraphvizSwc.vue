<template>
    <div id="content">
        <div class="btn">
            <el-button type="success" icon="el-icon-picture" circle @click="downloadSvg">download</el-button>
        </div>
...
export default {
    ...
    methods: {
        ...
        onImportExcel(file) {
            ...
            let selectedObj = {};
            let nodeArray = [];
            /**取到所有节点放到selectedSwc里面 */
            for (let i = 0; i < selectedNode.length; i++) {
                console.log(selectedNode[i]);
                if (selectedNode[i].hasOwnProperty("children")) {
                    //记录选择的RUNNABLE节点
                    ...
                    selectedObj = {};
                    selectedObj["swcName"] = selectedNode[i]["label"];
                    selectedObj["port"] = [];
                } else {
            ...
            for (let i = 0; i < sheet.length; i++) {
                let nodeObj = {};
                if (selectedSwc.indexOf(sheet[i][0]) >= 0) {
                    count++;
                    selectedPort.forEach((swc) => {
                        if (
                            swc["swcName"] === sheet[i][0] &&
                            swc["port"].indexOf(sheet[i][2]) >= 0 ) {   
                            if(selectedSwc.indexOf(sheet[i][1]) >= 0){
                            nodeObj["sendSwc"] = sheet[i][0];
                            nodeObj["recSwc"] = sheet[i][1];
            ...
            nodeArray.forEach((nodeObj) => {
                str += `${nodeObj["sendSwc"]}->${nodeObj["recSwc"]} [tooltip = "${nodeObj["portVal"]}" labelfontcolor="red" color = "skyblue" fontcolor = "purple"];\n`;
            });
