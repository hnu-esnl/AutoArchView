        ...
        onImportExcel(file) {
            ...
            for (let i = 0; i < selectedNode.length; i++) {
                if (selectedNode[i].hasOwnProperty("grandfather")) {
                    // if (selectedNode[i + 1]["label"] === "master") {
                    //     selectedNode[i]["master"] === true;
                    // } else {
                    //     selectedNode[i]["master"] == false;
                    // }
            ...
            for (let i = 0; i < selectedNode.length; i++) {
                //有head属性表明是发送接受swc
                if (
                    selectedNode[i].hasOwnProperty("head") &&
                    selectedSwc.indexOf(selectedNode[i]["label"]) === -1
                ) {
                    // console.log(selectedNode[i + 1], "s[i+1]matser");
                    if (
                        i < selectedNode.length - 1 &&
                        selectedNode[i + 1]["label"] === "master"
                    ) {
                        //看看该节点的下个节点是否为master节点，如果不是，表明当前的swc是主swc

                        selectedAttr.push("master");
                        selectedSwc.push(selectedNode[i]["label"]);
                        i++;
                    } else {
                        //非主swc
                        selectedAttr.push("slave");
                        selectedSwc.push(selectedNode[i]["label"]);
                    }
                } else if (
                    ...
                    if (i > 0 && selectedNode[i - 1]["label"] === "master") {
                        //看看该节点的上个节点是否为master节点，如果是，还是得判断它的下一个
                        selectedAttr.push("master");
                        selectedSwc.push(selectedNode[i]["label"]);
                    } else {
                        //非主swc
            ...
            for (let i = 0; i < selectedSwc.length; i++) {
                let swcName = selectedSwc[i];
                if (selectedAttr[i] === "master") {
                    console.log("进入了master");
                    str += `${swcName}[width=4.0 height=1.6 label = "{${swcName}| | }" ]\n`;
                } else {
                    str += `${swcName}[label = "{${swcName}| | }"]\n`;
                }
            }
            // selectedSwc.forEach((swcName) => {
            //     if (swcObj["attribute"] === "master") {
            //         str += `${swcObj["swc"]}[label = "{${swc}| | }" fixedsize = true width=7.104]\n`;
            //     } else {
            //         str += `${swcObj["swc"]}[label = "{${swc}| | }"]\n`;
            //     }
            // });
                ...
                for (let i = 0; i < selectedMsg.length; i++) {
                    ...
                    if (
                        ...
                        if (JSON.stringify(node) === "{}") {
                            ...
                            msgList.push(msg);
                            //记录左边的图
                            edgeStr += `${node["sendName"]}->${node["receiveName"]} [tooltip="${node["pPort"]}" xlabel = "${node["num"]}" color ="${this.colorList[colorIndex]}" fontcolor = "${this.colorList[colorIndex]}" ];\n`;
                            //记录右边的文字
                            svgTxt += `<tspan x="0" y=${
                                msgHeight++ * 15
                            ...
                            edgeStr += `${sendName}->${receiveName} [tooltip="${pPort}" xlabel = "${numEdge}" color ="${this.colorList[colorIndex]}" fontcolor = "${this.colorList[colorIndex]}" ]; \n`;
                            svgTxt += `<tspan x="0" y=${
                                msgHeight++ * 15
                            }  fill = ${
                                this.colorList[colorIndex]
            ...
            console.log("我是excel的data", this.excelData, str);
            return str + edgeStr;
        },
        downloadLeftPicSvg() {
            const { remote } = require("electron");
